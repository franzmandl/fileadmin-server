...
class FileResourceNativeTests(
    ...
    fun testRootDirectory() {
        ...
        try {
            ...
            assertThat(directory.inodes).hasSize(13)
        } finally {
    ...
    fun testExistingFile() {
        val result =
            mockMvc.perform(get("${ApplicationCtx.RequestMappingPaths.authenticated}/file?path=/.fileadmin.json"))
                ...
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andReturn()
        assertThat(result.response.contentAsString).isEqualTo(Path.of("./src/test/resources/jail2/.fileadmin.json").readText())
    }
    ...
    fun testExistingFileStreamWithRange() {
        val result =
            mockMvc.perform(get("${ApplicationCtx.RequestMappingPaths.authenticated}/file/stream?path=/.fileadmin.json").header("Range", "bytes=7-24"))
                ...
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andReturn()
        assertThat(result.response.contentAsString).isEqualTo(Path.of("./src/test/resources/jail2/.fileadmin.json").readText().substring(7, 25))
    }
